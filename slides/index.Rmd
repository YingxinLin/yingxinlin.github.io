...
To load the `scMerge` package. We designed our package to be consistent with the popular BioConductor’s single cell analysis framework, namely the `SingleCellExperiment` package and `scater`.
...
We will use a mouse ESC data as an example to illustrate  `scMerge` function. 
...
```{r results = "hide", fig.show = 'hide', cache = F}
...
```{r fig.height=5, fig.width=6, cache = F}
scater::plotPCA(sce_mESC, 
                colour_by = "cellTypes", 
                shape_by = "batch",
...
`RUVIII` function in the package `ruv` is great but slow for the large single-cell daat. We develop a fast version of scMerge, by using 
...
A figure indicates the speed up
